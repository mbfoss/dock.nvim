...
-- global default (the value new windows inherit), even for options with no real
-- global scope — so every panel open would leak its settings into future plain
-- windows. Force `scope = "local"`.
---@param win integer
---@param opt string
---@param val any
...
function M.create_window(buffer, enter, config, on_close)
    ...
    local augroup = vim.api.nvim_create_augroup("dock_window_#" .. win, { clear = true })
...
function M.create_scratch_buffer(listed, buffer_options, on_delete)
    ...
    if on_delete then
        vim.api.nvim_create_autocmd({ "BufDelete", "BufWipeout" }, {
            buffer   = buf,
            once     = true,
            callback = function() on_delete() end,
        })
    end
    return buf
end
