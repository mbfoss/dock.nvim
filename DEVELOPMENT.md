...
the `vim` API. busted must already be installed for Lua 5.1, the version Neovim
embeds:
...
`make test` fails if it is missing rather than installing anything. Specs are
`tests/*_spec.lua`; `tests/init.lua` is the shared helper.

## Help file

`doc/dock.txt` is generated from `README.md` with
[panvimdoc](https://github.com/kdheepak/panvimdoc), so the README is the single
source of prose and the help file follows it:
...
Needs pandoc (`brew install pandoc`). panvimdoc is fetched on first run and
cached under `$XDG_CACHE_HOME`, pinned to the commit in `PANVIMDOC_COMMIT` — a
tag can be moved, a commit cannot — so the help file is reproducible. Point
...
* Window options are set with an explicit `scope = "local"`. `vim.wo[win].opt =
  val` also writes Neovim's hidden global default, even for options with no real
  global scope, which would paint the dock's `'winbar'` onto unrelated windows.
